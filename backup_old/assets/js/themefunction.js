jQuery(document).ready(function() {
	...
	jQuery("#th-homeslider").owlCarousel({
		...
		pagination : false,
		navigationText: [
			"<span class='th-btncurveprev'><i class='fa fa-long-arrow-alt-left'></i></span>",
			"<span class='th-btncurvenext'><i class='fa fa-long-arrow-alt-right'></i></span>"
		],
	});
	/* ---------------------------------------
			STATISTICS
	...
	jQuery("#th-docteamslider").owlCarousel({
		...
		pagination : false,
		navigationText: [
			"<span class='th-btnsquareprev'><i class='fa fa-long-arrow-alt-left'></i></span>",
			"<span class='th-btnsquarenext'><i class='fa fa-long-arrow-alt-right'></i></span>"
		],
		itemsCustom : [
			[0, 1],
			[480, 2],
	...
	jQuery(".th-testimonialslider").owlCarousel({
		jsonPath: window.location.origin + "/assets/js/customImage.json",
		jsonSuccess: function (data) {
			var content = "";
			for(var i in data['items']){				
				content += "<div class='item'><div class='th-testimonial testimonial"+i+"'><div class='th-clientinfo'><h3>" + data['items'][i].name + "</h3><span>" + data['items'][i].case_title + "</span></div><blockquote><q>" + data['items'][i].desc + "</q></blockquote></div></div>";
				var imgPath = window.location.origin + '/assets/images/thumbnails/' + data.items[i].img;								
				var test_img = document.head.appendChild(document.createElement("style"));
				test_img.innerHTML = ".testimonial"+i+" blockquote:before {background: url("+ imgPath +") no-repeat 0 0; background-size: cover}";				
			}
			$('.th-testimonialslider').append(content);
		},
		slideSpeed : 300,
		paginationSpeed : 400,
		singleItem:true,
